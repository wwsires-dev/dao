	...
	**/
	public function getODataMetaData( array excludeKeys = variables.meta.privateKeys, ignoreCache = false, array tables = [], array dynamicProperties = [] ){
		...
		for( table in tables ){
			...
			// TODO: Why the heck is NORM[ table ].getIDField() evaluating to the "Status" field on production and ID here( because ID is correct )
			//		Maybe ID is correct but status is what CT client expects...???
			// WriteDump( var=[ local, arguments ], abort=true, showUDFs=false, label=getFunctionCalledName() );
	...
	**/
	private function generateODataProperties( array excludeKeys = variables.meta.privateKeys, array dynamicProperties = [] ){
		...
		for ( var col in properties ){
			...
			/* is part of a key? */
			if (
				col.KeyExists( "fieldType" )
				&& (
					col.fieldType == "id"
					|| col.KeyExists( "uniquekey" )
					|| col.name == getIDField()
				)
			) {
				prop["name"] = lcase( col.column );
